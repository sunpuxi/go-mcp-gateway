...
function Permissions() {
  ...
  const fetchData = useCallback(() => {
    ...
    Promise.all([getClients(), getTools(), getProjects()])
      .then(([c, t, p]) => {
        ...
        setLoading(false)
        if (c.length > 0) {
          setSelectedClient(c[0].client_id)
        }
      })
  ...
  useEffect(() => {
    ...
    getClientPermissions(selectedClient)
      .then(data => {
        setDraft(Array.isArray(data) ? data : [])
  ...
  const groupedTools = projects
    .map(proj => ({
      ...proj,
      tools: tools.filter(t => t.project_id === proj.project_id),
    }))
    .filter(g => g.tools.length > 0)

  if (loading) {
    return <div><div className="page-header"><h1>权限管理</h1></div><div style={{ textAlign: 'center', padding: 60, color: '#7f8c9b' }}>加载中...</div></div>
  }
      ...
      <div className="permission-layout">
        ...
        <div className="permission-tools">
          {!selectedClient ? (
            ...
            <div style={{ color: '#7f8c9b', padding: 20, textAlign: 'center' }}>加载权限中...</div>
          ) : (
            <>
